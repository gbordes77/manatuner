...
vi.mock('../../src/services/deckAnalyzer', async (importOriginal) => ({
...
describe('AnalyzerPage', () => {
  beforeEach(() => {
    ...
    DeckAnalyzer.analyzeDeck.mockImplementation(() => Promise.resolve(mockAnalysisResult))
  })
  ...
  it("affiche les onglets d'analyse", async () => {
    ...
    const tabNames = allTabs.map((tab) => tab.getAttribute('aria-label') || tab.textContent)
  ...
  it("affiche l'erreur d'analyse et permet de réessayer", async () => {
    DeckAnalyzer.analyzeDeck.mockRejectedValueOnce(new Error('Metadata service unavailable'))
    renderWithProviders(<AnalyzerPage />)
    fireEvent.change(screen.getByPlaceholderText(/paste your decklist here/i), {
      target: { value: '20 Forest' },
    })
    fireEvent.click(screen.getByRole('button', { name: /analyze/i }))
    expect(
      await screen.findByText(/Failed to analyze deck: Metadata service unavailable/)
    ).toBeInTheDocument()
    expect(screen.queryByTestId('analysis-results')).not.toBeInTheDocument()
    expect(screen.getByRole('button', { name: /analyze/i })).toBeEnabled()
    fireEvent.click(screen.getByRole('button', { name: /analyze/i }))
    expect(await screen.findByTestId('analysis-results')).toBeInTheDocument()
  })
  ...
  it('transmet le brouillon au store persistant et le restaure au remontage', async () => {
    const view = renderWithProviders(<AnalyzerPage />)
    const draft = '12 Forest\n12 Island'
    fireEvent.change(screen.getByPlaceholderText(/paste your decklist here/i), {
      target: { value: draft },
    })
    await waitFor(() => expect(view.store.getState().analyzer.deckList).toBe(draft))
    view.unmount()
    renderWithProviders(<AnalyzerPage />, { store: view.store })
    expect(screen.getByPlaceholderText(/paste your decklist here/i)).toHaveValue(draft)
  })
