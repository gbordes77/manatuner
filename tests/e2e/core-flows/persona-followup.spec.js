...
const basics = [
  { name: 'Plains', mana_cost: '', cmc: 0, type_line: 'Basic Land — Plains', oracle_text: '({T}: Add {W}.)', colors: [], color_identity: ['W'], produced_mana: ['W'], layout: 'normal' },
...
async function fixture(page, cards = basics) {
  await page.route('https://api.scryfall.com/**', async route => {
    ...
    if (request.url().includes('/cards/collection')) {
      ...
      await route.fulfill({ json: { object: 'list', data: cards.filter(c => names.includes(c.name.toLowerCase())) } })
    } else {
      const url = new URL(request.url())
      const card = cards.find(c => c.name === (url.searchParams.get('exact') || url.searchParams.get('fuzzy')))
...
test('T04 home exact example starts supported and agrees with independent opening-hand oracle', async ({ page }) => {
  await fixture(page)
  await page.goto('/')
  await page.locator('a[href="/analyzer?sample=exact"]').click()
  await expect(page.getByPlaceholder(/paste your decklist/i)).toHaveValue('24 Plains\n36 Savannah Lions')
  await analyze(page)
  await expect(page.getByRole('button', { name: 'Exact goldfish potential', exact: true })).toHaveAttribute('aria-pressed', 'true')
  const oracle = 1 - Array.from({ length: 7 }, (_, i) => (36 - i) / (60 - i)).reduce((a, b) => a * b, 1)
...
test('T07 JSON and CSV retain deck identity, totals, engine and score definitions', async ({ page }, testInfo) => {
  await fixture(page)
  ...
  const bodies = {}
  for (const [kind, label] of [['json', 'JSON (Backup)'], ['csv', 'CSV (Sheets / Pandas)']]) {
  ...
  expect(Object.keys(data.scoreDefinitions)).toEqual(expect.arrayContaining(['health', 'blueprint', 'mulligan', 'limitation']))
...
test('T07 real clipboard share opens in a separate clean browser context', async ({ page, browser }, testInfo) => {
  await fixture(page)
  ...
  try {
    ...
    await expect(second.getByPlaceholder(/paste your decklist/i)).toHaveValue('24 Plains\n36 Savannah Lions')
...
for (const width of [360, 390, 768, 1440]) {
  test(`T08 exact example keyboard and viewport ${width} in light and dark themes`, async ({ page }, testInfo) => {
    await fixture(page)
    ...
    for (const colorScheme of ['light', 'dark']) {
      ...
      expect(await page.evaluate(() => ({ width: innerWidth, overflow: document.documentElement.scrollWidth > innerWidth }))).toEqual({ width, overflow: false })
      await page.evaluate(async () => {
        await document.fonts.ready
        await Promise.all(document.getAnimations().filter(a => a.effect?.getComputedTiming().iterations !== Infinity).map(a => a.finished.catch(() => {})))
      })
      const resultContrast = await new AxeBuilder({ page })
        .include('#quick-verdict').include('[data-testid="analysis-results"] > .MuiTypography-root')
        .withRules(['color-contrast']).analyze()
      await testInfo.attach(`${width}-${colorScheme}-result-contrast`, { body: JSON.stringify(resultContrast), contentType: 'application/json' })
      expect(resultContrast.violations, JSON.stringify(resultContrast.violations, null, 2)).toEqual([])
      await page.screenshot({ path: testInfo.outputPath(`${width}-${colorScheme}.png`), fullPage: true })
      ...
      const deckContrast = await new AxeBuilder({ page }).include('#deck-editor').withRules(['color-contrast']).analyze()
      await testInfo.attach(`${width}-${colorScheme}-deck-contrast`, { body: JSON.stringify(deckContrast), contentType: 'application/json' })
...
test('T03 real midrange persists explicit unsupported reasons across reload', async ({ page }) => {
  ...
  expect(Object.values(before.analysis.unsupportedSpellAnalysis).join(' ')).toMatch(/Abandoned Air Temple|unsupported|not modeled|not supported/i)
  await page.reload()
  expect((await saved(page))[0]).toEqual(before)
})


test('T03 compatible 24 and 20 Plains builds compare actual saved probabilities', async ({ page, browser }) => {
  const records = []
  for (const lands of [24, 20]) {
    const context = await browser.newContext({ baseURL: test.info().project.use.baseURL })
    try {
      const build = await context.newPage()
      await build.addInitScript(() => localStorage.setItem('manatuner-onboarding-completed', 'true'))
      await fixture(build)
      await build.goto('/analyzer?sample=exact')
      await build.getByLabel('Deck Name (optional)').fill(`White ${lands}`)
      await build.getByPlaceholder(/paste your decklist/i).fill(`${lands} Plains\n${60 - lands} Savannah Lions`)
      await analyze(build)
      records.push((await saved(build))[0])
    } finally { await context.close() }
  }
  expect(records[0].analysis.spellAnalysis['Savannah Lions'].percentage).toBeGreaterThan(records[1].analysis.spellAnalysis['Savannah Lions'].percentage)
  await page.goto('/my-analyses')
  await page.evaluate(records => localStorage.setItem('manatuner_analyses', JSON.stringify(records)), records)
  await page.reload()
...
test('T05 multiplayer control exposes the free redraw contract and toggles without simulation', async ({ page }) => {
  await fixture(page)
  await page.goto('/analyzer?sample=exact')
  await analyze(page)
  ...
  const multiplayer = page.getByRole('checkbox', { name: 'Multiplayer: free first mulligan and turn-one draw' })
...
test('T04/T07 public Atraxa example keeps 99 plus commander, four colors, EDH horizon and CSV zones', async ({ page }, testInfo) => {
  ...
  await page.getByTestId('tab-blueprint').click()
  for (const [kind, label] of [['csv', 'CSV (Sheets / Pandas)'], ['json', 'JSON (Backup)']]) {
    await page.getByRole('button', { name: 'Export Blueprint', exact: true }).click()
    const pending = page.waitForEvent('download')
    await page.getByRole('menuitem', { name: label, exact: true }).click()
    ...
    if (kind === 'csv') {
      ...
      expect(body.split('\n').find(line => line.startsWith('deck,"Atraxa,'))).toMatch(/,false,true\r?$/)
      expect(body.split('\n').find(line => line.startsWith('deck,Savannah Lions,'))).toMatch(/,true,false\r?$/)
    } else {
...
test('T05 real 3k worker distinguishes free and paid mulligan results and guidance', async ({ page }, testInfo) => {
  await fixture(page)
  await page.goto('/analyzer?sample=exact')
  await analyze(page)
  await page.getByTestId('tab-mulligan').click()
  ...
  await expect(page.getByText(/Before the free mulligan, the first redraw still keeps seven cards/)).toBeVisible()
  await expect(page.getByText(/After the free mulligan, keep-seven threshold: \d+/)).toBeVisible()
  await expect(page.getByText('(After the free redraw and one paid mulligan)', { exact: true })).toBeVisible()
  await expect(page.getByText('(After the free redraw and two paid mulligans)', { exact: true })).toBeVisible()
  await page.screenshot({ path: testInfo.outputPath('multiplayer-3k.png'), fullPage: true })
  await multiplayer.uncheck()
  await expect(page.getByText(/Based on 3,000 samples per hand size/)).toBeVisible()
  await expect(page.getByText(/A paid mulligan draws seven again; keep six after bottoming one card/)).toBeVisible()
  await expect(page.getByText('(After one paid mulligan)', { exact: true })).toBeVisible()
  await expect(page.getByText('(After the free redraw and one paid mulligan)', { exact: true })).toHaveCount(0)
