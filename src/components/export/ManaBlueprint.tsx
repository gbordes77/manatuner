...
export const ManaBlueprint: React.FC<ManaBlueprintProps> = ({
  ...
  const detectedFormat =
    format ||
    (analysisResult.totalCards <= 45
      ? 'Limited'
      : analysisResult.totalCards >= 99
        ? 'Commander'
        : 'Constructed')

  // Get active colors from the deck
  const activeColors = (Object.keys(analysisResult.colorDistribution) as WUBRGColor[]).filter(
    (color) => analysisResult.colorDistribution[color] > 0
  )
