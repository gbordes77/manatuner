...
export const CastabilityTab: React.FC<CastabilityTabProps> = memo(
  ({ analysisResult, initialModel = 'estimate' }) => {
    ...
    const [probabilityModel, setProbabilityModel] = useState<'estimate' | 'exact'>(initialModel)
            ...
            effectiveCards
              .filter(
                (card) =>
                  card.isLand &&
                  !card.isCommander &&
                  card.producedMana?.includes(color as ManaColor)
              )
    ...
    return (
      <>
        ...
        >
          Each card shows on-curve cast % — scroll the list. Priority horizon spells are
          highlighted.
        </Typography>
        ...
        {detectedFamily && analysisResult && (
          <Alert severity="info" sx={{ mb: 2 }} data-testid="format-family-banner">
            ...
            <Typography variant="caption" display="block" sx={{ mt: 0.5, opacity: 0.9 }}>
              Format controls above set ramp/removal model. Change Format anytime; click the Auto
              chip if you locked a format and want detection again.
            </Typography>
          </Alert>
        )}
        ...
        {Object.keys(analysisResult.unsupportedSpellAnalysis ?? {}).length > 0 && (
          <Alert severity="info" sx={{ mb: 2 }}>
            The saved lands-only calculation excludes some mechanics in this deck:{' '}
            {[...new Set(Object.values(analysisResult.unsupportedSpellAnalysis ?? {}))].join('; ')}.
            Exact mode also has coverage limits. Keep Mana estimates for an approximation, or{' '}
            <a href="/analyzer?sample=exact">try the basic-land exact example</a>.
          </Alert>
        )}
        <ToggleButtonGroup
          exclusive
            ...
            {boardScope === 'postboard' && activeSwaps.length > 0 && (
              <Box sx={{ mt: 1, p: 1.5, bgcolor: 'info.main', borderRadius: 1, opacity: 0.9 }}>
                <Typography variant="caption" color="info.contrastText" fontWeight={600}>
                  Post-board castability — {activeSwaps.reduce((s, sw) => s + sw.quantity, 0)}{' '}
                  card(s) swapped in from sideboard
                </Typography>
              </Box>
            )}
            {boardScope === 'postboard' &&
