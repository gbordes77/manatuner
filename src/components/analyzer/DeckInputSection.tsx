...
export const DeckInputSection: React.FC<DeckInputSectionProps> = memo(
  ({
    ...
    const deckInputRef = useRef<HTMLTextAreaElement>(null)
    const focusOnExpand = useRef(false)
    useEffect(() => {
      if (!isDeckMinimized && (autoFocusDeckList || focusOnExpand.current)) {
        deckInputRef.current?.focus()
        focusOnExpand.current = false
      }
    }, [autoFocusDeckList, isDeckMinimized])
    ...
    return (
      ...
      >
        ...
        <Box id={editorId} hidden={Boolean(isDeckMinimized && analysisResult)}>
        ...
        {isDeckMinimized && analysisResult && (
          <Box sx={{ textAlign: 'center', py: 2 }}>
            ...
            <Button
              ...
              sx={{ '&:focus-visible': { outline: '3px solid #1565c0', outlineOffset: '3px' } }}
            >
              Edit Deck
            </Button>
          </Box>
