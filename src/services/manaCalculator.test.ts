...
describe('ManaCalculator', () => {
  ...
  describe('Calculs mathématiques de base', () => {
    test('hypergeometric PMF correct (validates binomial internally)', () => {
      // P(X=3) for 60-card deck, 24 lands, 7 cards drawn, exactly 3 lands
      const prob = calculator.hypergeometric(60, 24, 7, 3)
      expect(prob).toBeCloseTo(0.3086, 3)
      // Edge cases
      expect(calculator.hypergeometric(60, 24, 7, -1)).toBe(0)
      expect(calculator.hypergeometric(60, 24, 7, 25)).toBe(0) // impossible: can't draw 25 from 7
    })
