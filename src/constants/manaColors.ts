...
/** WUBRG mana colors (no Colorless) -- used for styling iteration */
export const MANA_COLORS = ['W', 'U', 'B', 'R', 'G'] as const

/** Type for the 5 WUBRG colors only (subset of ManaColor, excludes 'C') */
export type WUBRGColor = (typeof MANA_COLORS)[number]
...
/**
 * Complete style definitions for WUBRG mana colors.
 * Includes background, text, border colors and full name.
 */
export const MANA_COLOR_STYLES: Record<WUBRGColor, ManaColorStyle> = {
...
/** Mapping of WUBRG mana color codes to their full names */
export const COLOR_NAMES: Record<WUBRGColor, string> = {
  W: 'White',
  U: 'Blue',
  B: 'Black',
  R: 'Red',
...
export function getManaColorStyle(color: string): ManaColorStyle | undefined {
  return MANA_COLOR_STYLES[color as WUBRGColor]
}
